...
public class DataStore {
	...
	/*	try {
		...
		try {
			Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/jid_thrillio?useSSL=false", "root", "root");
	...
	private static void loadWebLinks(Statement stmt)throws SQLException{
		...
		while (rs.next()) {
			...
			Bookmark bookmark = BookmarkManager.getInstance().createWebLink(id, title, "", url,host/*, values[7]*/);
