...
public class View {
			...
			for (Bookmark bookmark : bookmarkList) {
				...
				if (user.getUserType().equals(UserType.EDITOR)
					...
					if (bookmark.isKidFriendlyEligible()
							&& bookmark.getKidFriendlyStatus().equals(
	 ...
	 * 
	 * for(int i=0; i<DataStore.USER_BOOKMARK_LIMIT;i++){ int typeOffset = (int)
	 * (Math.random() * DataStore.BOOKMARK_TYPES_COUNT); int bookmarkOffset =
	 * (int) (Math.random() * DataStore.BOOKMARK_COUNT_PER_TYPE); Bookmark
	 * bookmark = bookmarks[typeOffset][bookmarkOffset];
